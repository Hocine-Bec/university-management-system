...
public class StudentService : IStudentService
{
    ...
    public StudentService(IStudentRepository repository, IMapper mapper, IMyLogger logger, IValidationService validator)
    {
    ...
    public async Task<Result<StudentResponse>> AddAsync(StudentRequest request)
    {
        ...
        if (!validationResult.IsSuccess)
            return Result<StudentResponse>.Failure(validationResult.Error, validationResult.ErrorType);
    ...
    public async Task<Result> UpdateStudentStatusAsync(int id, UpdateStudentStatusRequest request)
    {
        var validationResult = await _validator.ValidateAsync(request);
        if (!validationResult.IsSuccess)
            return Result.Failure(validationResult.Error, validationResult.ErrorType);
        ...
        try
        {
            ...
            student.Notes = string.IsNullOrEmpty(request.Notes) ? student.Notes : request.Notes;
        ...
        {
            var statusName = request.ToString();
